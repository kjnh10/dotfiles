...
""""""""""""""""""""""""""""""
	...
	"specfying buffers where neocomplete dosen't work.
	...
	" vimfiler上でのキーマッピング
		...
		"移動
			" nmap <silent><buffer> i <Plug>(vimfiler_loop_cursor_up)
			" nmap <silent><buffer> k <Plug>(vimfiler_loop_cursor_down)
			" nmap <silent><buffer> j <Nop>
			" nmap <silent><buffer> l <Nop>
			nmap <silent><buffer> <c-i> <Plug>(vimfiler_switch_to_another_vimfiler)
		...
		"その他
			...
			nmap <silent><buffer> o <Plug>(vimfiler_expand_tree)
			...
			nnoremap <silent><buffer><expr> R
					\ vimfiler#do_switch_action('rec/async')
...
autocmd MyAutoCmd bufnewfile,bufread *.scpt,*.applescript :setl filetype=applescript
autocmd MyAutoCmd FileType vb :set commentstring='%s

"withever用の設定
" this setttig open evernote when starting vim. not investigated yet
"	set runtimepath+=~/withever.vim/
	nmap <leader>en <plug>evimerCreatenote
	nmap <leader>eu <plug>(withever_update)

nnoremap <f3> :call copy(expand("%:p"))<CR>
